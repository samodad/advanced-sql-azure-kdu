...
--CREATE VIEWS
--View: Yearly Total Applications
CREATE VIEW vw_YearlyTotals AS
SELECT 
    Year,
...
CREATE VIEW vw_ApplicationTypeSummary AS
SELECT 
    Year,
    Application_Type,
    SUM(Applications) AS TotalApplications
...
--Geographic Region Trends
CREATE VIEW vw_RegionTrends AS
SELECT
    Year,
    Region,
    SUM(Applications) AS TotalApplications
